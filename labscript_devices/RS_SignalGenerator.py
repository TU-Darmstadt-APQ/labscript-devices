...
MAX_FREQUENCY = 1000000000  # in Hz
...
class RS_SignalGeneratorTab(DeviceTab):
    ...
    def initialise_GUI(self):
        ...
        self.ramping_target_value = 190.000  # MHz
...
class RS_SignalGeneratorWorker(GPIBWorker):
    ...
    def send_GPIB_frequency(self, frequency=None):
        ...
        if frequency is not None:
            sendStr = "RF "
            sendStr += "{0:.6f}".format(float(frequency) / 1E6)  # first digit is 10GHz, so print leading zeros!
            sendStr += "MHZ"
    ...
    def program_manual(self, front_panel_values):
        ...
        self.send_GPIB_frequency(frequ * 1E6)  # convert MHz to Hz
