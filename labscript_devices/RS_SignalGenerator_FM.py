...
class RS_SignalGeneratorTab(DeviceTab):
    ...
    def initialise_GUI(self):
        ...
        ramping_validator = QDoubleValidator(MIN_FREQUENCY / 1E3, MAX_FREQUENCY / 1E3, 3)  # kHz
...
class RS_SignalGeneratorWorker(GPIBWorker):
    ...
    def send_GPIB_frequency(self, frequency=None):
        ...
        if frequency is not None:
            sendStr = "FM:INT:FREQ "
            sendStr += "{0:.6f}".format(float(frequency) / 1E3)  # first digit is 10GHz, so print leading zeros!
            sendStr += "kHZ"
            # self.send_string('FM:INT:FREQ 50kHZ')
            self.GPIB_connection.write(sendStr)

    def program_manual(self, front_panel_values):
        frequ = front_panel_values['frequency']  # in MHz!

        self.send_GPIB_frequency(frequ * 1E3)  # convert kHz to Hz
        self.send_string('FREQ 90MHZ')
        self.send_string('OUTP ON')
        self.send_string(':FM:STAT ON')
        self.send_string(':FM:SOUR INT')
        self.send_string(':FM:BAND WIDE')
        self.send_string(':FM:DEV 125kHZ')
    ...
    def transition_to_buffered(self, device_name, h5_filepath, initial_values, fresh):
        ...
        frequency = initial_values['frequency'] * 1e3  # convert to Hz
        ...
        self.send_GPIB_frequency(frequency)
