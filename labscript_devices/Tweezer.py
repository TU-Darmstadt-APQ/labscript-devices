...
class Tweezer(TriggerableDevice):
    description = 'Tweezer '
    ...
    @set_passed_properties(
        property_names={
            ...
            "device_properties": ["amplitude", "rise_time", "fall_time", "wait_time", "remove_mode", "remove_speed_multiplier"]}
    )

    def __init__(self, name, parent_device, connection,
                 Tweezer_port=1027, ramp_speed=1000, amplitude=5, rise_time=300e-6, fall_time=300e-6, wait_time=100e-6, remove_mode='default', remove_speed_multiplier=2, trigger_edge_type='rising', **kwargs):
    ...
    def rearrange(self, t, res_grid, routing_mode='default', ramp_speed=None):
        self._trigger(t)
        if ramp_speed is None:
            ramp_speed = self.ramp_speed
        self.instructions[t] = {'type': 'rearrange', 'res_grid': res_grid, 'mode': routing_mode, 'ramp_speed': ramp_speed}

    def compress(self, t, res_grid, compression_mode='default', ramp_speed=None):
        self._trigger(t)
        if ramp_speed is None:
            ramp_speed = self.ramp_speed
        self.instructions[t] = {'type': 'compress', 'res_grid': res_grid, 'mode': compression_mode, 'ramp_speed': ramp_speed}

    def move(self, t, move_list, ramp_speed=None):
        self._trigger(t)
        if ramp_speed is None:
            ramp_speed = self.ramp_speed
        self.instructions[t] = {'type': 'move_list', 'move_list': [[pos1[0], pos1[1], pos2[0], pos2[1], amp] for pos1, pos2, amp in move_list], 'mode': '', 'ramp_speed': ramp_speed}

    def generate_code(self, hdf5_file):
        group = self.init_device_group(hdf5_file)

        instructions = []
        for i, t in enumerate(sorted(list(self.instructions.keys()))):
            instruction = self.instructions[t]

            if (instruction['type'] == 'rearrange') or (instruction['type'] == 'compress'):
                group.create_dataset('Grid{}'.format(i), data=instruction['res_grid'])
            elif instruction['type'] == 'move_list':
                group.create_dataset('Moves{}'.format(i), data=np.asarray(instruction['move_list']))

            instructions.append((t, instruction['type'], instruction['mode'], instruction['ramp_speed']))
        instructions_array = np.array(instructions, dtype=[('time', float), ('type', 'a256'), ('mode', 'a256'), ('ramp_speed', int)])
        group.create_dataset('Instructions', data=instructions_array)
...
class TweezerWorker(Worker):
    ...
    def transition_to_buffered(self, device_name, h5file, initial_values, fresh):
        with h5py.File(h5file, 'r') as hdf5_file:
            ...
            remove_mode = device_properties['remove_mode'].decode('utf-8') if isinstance(device_properties['remove_mode'], bytes) else device_properties['remove_mode']
            remove_speed_multiplier = device_properties['remove_speed_multiplier']

            group = hdf5_file['devices'][device_name]
            instructions = []
            print(1)
            for i, instruction in enumerate(group['Instructions']):
                typ = instruction['type'].decode('utf-8') if isinstance(instruction['type'], bytes) else instruction['type']
                mode = instruction['mode'].decode('utf-8') if isinstance(instruction['mode'], bytes) else instruction['mode']
                ramp_speed = instruction['ramp_speed']
                if (typ == 'rearrange') or (typ == 'compress'):
                    data = np.array(group['Grid{}'.format(i)])
                elif typ == 'move_list':
                    data = [[(elenment[0], elenment[1]), (elenment[2], elenment[3]), int(elenment[4])] for elenment in np.array(group.get('Moves{}'.format(i)))]
                else:
                    data = None
                instructions.append((typ, mode, data, ramp_speed))
        settings_dict = {'amplitude': amplitude, 'rise_time': rise_time, 'fall_time': fall_time, 'wait_time': wait_time, 'instructions': instructions, 'remove_mode': remove_mode, 'remove_speed_multiplier': remove_speed_multiplier}
