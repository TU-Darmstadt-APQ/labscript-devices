...
class NI_DAQmxTab(DeviceTab):
    def initialise_GUI(self):
        ...
        with h5py.File(connection_table.filepath, 'r') as f:
            ...
            jumps = f['jumps']
            jump_device = jumps.attrs['jump_device']
        ...
        if jump_device == self.device_name:
            self.create_worker(
                "jump_worker",
                'labscript_devices.NI_DAQmx.blacs_workers.NI_DAQmxJumpWorker',
                {
                    'name': self.device_name + '_jumper',
                    'MAX_name': self.MAX_name,
                    'jump_address': str(self.settings['connection_table'].jump_device_address),
                    'Vmin': AO_base_min,
                    'Vmax': AO_base_max,
        ...
        if num_AI > 0:
            self.create_worker(
                ...
                {
                    'name': self.device_name + '_acqusition',
                    'MAX_name': self.MAX_name,
                    'jump_address': str(self.settings['connection_table'].jump_device_address),
                    'num_AI': num_AI,
                    'AI_chans': AI_chans,
