...
class NIBoard(IntermediateDevice):
    ...
    def __init__(self, name, parent_device, clock_terminal=None, MAX_name=None, sample_rate_AI=0, analog_in_range=None, **kwargs):
        ...
        self.analog_in_range = {}
        if type(analog_in_range) in (list, tuple):
            for channel in range(self.num_AI):
                self.analog_in_range['ai' + str(channel)] = analog_in_range
        elif type(analog_in_range) is dict:
            for channel in range(self.num_AI):
                if 'ai' + str(channel) in analog_in_range:
                    # apply ranges for each channel
                    self.analog_in_range['ai' + str(channel)] = analog_in_range['ai' + str(channel)]
                else:
                    # fill missing channels with max range
                    self.analog_in_range['ai' + str(channel)] = (-10, 10)
        else:
            # if no valid analog_in_range is passed, set range to max range for all channels
            for channel in range(self.num_AI):
                self.analog_in_range['ai' + str(channel)] = (-10, 10)

    @property
    def n_digital_lines_per_port(self):
        assert self.num_DO % self.num_ports_DO == 0, 'The number of digital lines defined for device class %s is not divisible by the number of digital ports.' % self.__class__.__name__
        return self.num_DO // self.num_ports_DO
    ...
    def convert_bools_to_bytes(self, digitals):
        ...
        for output in digitals:
            ...
            outputarray[self.n_digital_lines_per_port * port + line] = output.raw_output
    ...
    def generate_code(self, hdf5_file):
        ...
        if self.num_AI > 0:
            self.set_property('analog_in_range', self.analog_in_range, location='device_properties')
...
class RunviewerClass(object):
    ...
    def __init__(self, path, device, num_DO=None, num_ports_DO=None):
        self.path = path
        ...
        if num_DO is not None:
            self.num_DO = num_DO
        elif "num_DO" in connection_table_properties:
            self.num_DO = connection_table_properties["num_DO"]
        else:
            self.num_DO = 32

        if num_ports_DO is not None:
            self.num_ports_DO = num_ports_DO
        elif "num_ports_DO" in connection_table_properties:
            self.num_ports_DO = connection_table_properties["num_ports_DO"]
        else:
            self.num_ports_DO = 1
        ...
        k = 0
        for i in range(self.num_ports_DO):
            for j in range(self.n_digital_lines_per_port):
