...
class HP_6622A(IntermediateDevice):
    ...
    def generate_code(self, hdf5_file):
        ...
        for i in range(2):
            i += 1
            if output_table['v%d' % i] < MIN_VOLTAGE_80W_HIGH_RANGE or output_table['v%d' % i] > MAX_VOLTAGE_80W_HIGH_RANGE:
                raise LabscriptError("The voltage specified for {:s} is not within the power supply's voltage range".format(device.connection))
        for i in range(2):
            i += 1
            if output_table['c%d' % i] < MIN_CURRENT_80W_HIGH_RANGE or output_table['c%d' % i] > MAX_CURRENT_80W_HIGH_RANGE:
                raise LabscriptError("The voltage specified for {:s} is not within the power supply's voltage range".format(device.connection))
...
class HP_6622ATab(DeviceTab):
    ...
    def initialise_GUI(self):
        ...
        self.num_outputs = 2  # connection_table_properties['num_outputs']
