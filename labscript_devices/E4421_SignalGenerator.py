...
MIN_FREQUENCY = 100000  # in Hz
MAX_FREQUENCY = 3000000000  # in Hz


class E4421_SignalGenerator(IntermediateDevice):
    allowed_children = [StaticAnalogQuantity]

    description = 'Agilent E4421B synthesized signal generator'

    def __init__(self, name, GPIB_address = None, IP_address = None, **kwargs):
    ...
    def generate_code(self, hdf5_file):
        # do checks
        output_frequency = 200000000  # 3000000000# in Hz
        output_range = -1
...
@BLACS_tab
class E4421_SignalGeneratorTab(DeviceTab):
    ...
    def initialise_GUI(self):
        ...
        self.ramping_step_size = 0.02  # MHz
        self.ramping_delay = 100  # ms
        self.ramping_target_value = 424.000  # MHz
        ...
        self.level_base_min = -135
        self.level_base_max = 10
        self.level_base_step = 1
        ...
        if self.GPIB_address[0] != 'G':
            self.IP_address = self.GPIB_address
        else:
    ...
    def initialise_workers(self):
        if self.IP_address is not None:
            ...
            self.create_worker("main_worker", E4421_SignalGeneratorWorker_socket, worker_initialisation_kwargs)
        else:
            worker_initialisation_kwargs = {'GPIB_address': self.GPIB_address}
            self.create_worker("main_worker", E4421_SignalGeneratorWorker, worker_initialisation_kwargs)
...
class E4421_SignalGeneratorWorker(GPIBWorker):
    HARD_CODED_FREQ_LIM = 1.7e9 # hard coded Frequency limit to protect the EOM

    def send_GPIB_frequency(self, frequency=None, ref=2, smart_mode=True):
        ...
        # if smart_mode is True: an automatic ramping is enabled
        ...
        #     frequency = int(frequency)  # cast frequency to int!
        #     # frequency in Hz!!
        #     sendStr = ":ROSCillator:SOURCe:AUTO ON"  # Use external Reference is Standard for this device
        #     self.GPIB_connection.write(sendStr)
        # elif ref == 1:
        #     frequency = int(frequency)  # cast frequency to int!
        #     # frequency in Hz!!
        #     sendStr = ":ROSCillator:SOURCe:AUTO OFF"  # Use internal Reference
        #     self.GPIB_connection.write(sendStr)
        ...
        # self.GPIB_connection.write("REMOTE 710")

        # print("Remote Set")

        # self.GPIB_connection.write("*IDN?")
        if smart_mode is True:
            self.advanced_send_GPIB_frequency(frequency)
            pass
        else:
            if frequency is not None:
                if frequency > self.HARD_CODED_FREQ_LIM:
                    print("!!! WARNING !!!: synth got frequency of {}MHz but limit is at {}MHz --> used max Freq instead !!! WARNING !!!".format(frequency/1e6, self.HARD_CODED_FREQ_LIM/1e6))
                    frequency = self.HARD_CODED_FREQ_LIM
                sendStr = "FREQuency "+str(frequency/1e6)+" MHz"#"FREQency " + str(frequency) + " Hz"
                self.GPIB_connection.write(sendStr)
        pass
    def advanced_send_GPIB_frequency(self, 
                                     frequency:float=None, 
                                     ramp_step_size:float=0.2, 
                                     ramp_delay:float=100, 
                                     check_freq_continuous:bool=False
                                     ) -> None:
        """automatically slowly ramps synthesiser to desired frequency

        Parameters
        ----------
        frequency : float
            Frequency in MHz the synthesiser should be set to. If nothing is passed, no frequency change will happen.
        ramp_step_size : float
            ramp step size in MHz the system should use
        ramp_delay : float
            ramp delay between step in ms
        check_freq_continuous : bool
            if this is True the system ensures each step the right frequency is set (by asking the device via GPIB)
        """
        ...
        done_ramping = False
        frequency_MHz = frequency / 1e6 # convert frequency in MHz
        start_freq = self.get_curr_frequency() / 1e6
        print("start-freq = {} MHz, to freq = {} MHz --> ramping".format(round(start_freq, 1), round(frequency_MHz, 1)))
        # calculate how many frequencies should be send to the 

        calculated_num_of_steps = int(np.ceil(abs(frequency_MHz - start_freq)/ramp_step_size))
        progress_bar = tqdm.tqdm(total=abs(frequency_MHz - start_freq),
                                 desc="ramping synth",
                                 unit="MHz"
                                 )
        for run_number in range(calculated_num_of_steps):
            # get/compute the current frequency
            if frequency_MHz > start_freq:
                # ramp upwards
                current_freq = start_freq + run_number * ramp_step_size
            else:
                # ramp downwards
                current_freq = start_freq - run_number * ramp_step_size
            # calculate the frequency delta
            freq_delta = frequency_MHz - current_freq
            # compute the ramp step size in MHz
            if abs(freq_delta) > ramp_step_size:
                # step that is currently needed is too big to do in one step
                if freq_delta > ramp_step_size:
                    do_ramp_step_mhz = ramp_step_size
                else:
                    do_ramp_step_mhz = -ramp_step_size
            else:
                # only one step apart from goal frequency
                do_ramp_step_mhz = freq_delta
            new_freq_2_send_mhz = current_freq + do_ramp_step_mhz
            # print("sending {}MHz".format(new_freq_2_send_mhz))
            self.send_GPIB_frequency(int(new_freq_2_send_mhz * 1e6), smart_mode=False) # * 1e6 to convert into Hz
            progress_bar.update(round(abs(do_ramp_step_mhz), 1))
            if run_number != calculated_num_of_steps - 1:
                time.sleep(ramp_delay * 1e-3)
            pass
        progress_bar.close()
        pass

    def get_curr_frequency(self):
        """returns synthesizers current frequency in Hz"""
        self.GPIB_connection.write("FREQuency?")
        got_back_msg = self.GPIB_connection.read()
        got_back_freq = float(got_back_msg)
        return got_back_freq
    def program_manual(self, front_panel_values):
        frequ = front_panel_values['frequency'] * 1e6 # in MHz!
        # refSignal = front_panel_values['ref']  # Int or Ext ref signal
        print(frequ)

        self.send_GPIB_frequency(frequ)#, refSignal)  # convert MHz to Hz
    ...
    def transition_to_buffered(self, device_name, h5file, initial_values, fresh):
        ...
        self.send_GPIB_frequency(frequency)#, refSignal)
